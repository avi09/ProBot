...
def do_task(s, key, log, reference_keywords):
	...
	try:
		...
		if key == "mean":
			log.text += "The meaning of " + str(s[-1]) + " is - " + data_source["mean"][s[-1]][0] + "\n"
			return
		...
		if key == "googl":
			...
			while (not s == []) and (s[-1] not in reference_keywords):
				...
				s = s[:-1]
			random.shuffle(data_source["1"])
			log.text+= data_source["1"][1]
...
def command(s, log, data):
	...
	if reference_keywords[0][0] > 0:
		...
		random.shuffle(data_source["2"])
		log.text += data_source["2"][1]
		return
